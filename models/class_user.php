...
class User{
   ...
   */
    public function isloggedin($user_name){
        if(!(isset($user_name) && $user_name!='') ){
            header("location:../public/index.php"); // make this return true for more flexabilit
        }
  ...
  /**
	 ...
	 *
	 * @param $user_name gets all users friends from database
	 * @return object(stdClass) error message on failure / object of user's data on success
	 */
  public function get_all_user_data(){
	 ...
	 * Get user's friends
	 *
	 * @param $user_name User's name from login
	 * @return object(stdClass) error message on failure / object of user's data on success
	 */
	public function get_user_friends($user_name){
		...
		$query = "SELECT FriendB FROM $table WHERE FriendA = '$user_name';";
  ...
  public function register_user($user_display_name, $user_pass, $user_email, $user_active){
		global $db;
    ...
    $user_pass = password_hash($user_pass, PASSWORD_BCRYPT);
		...
		$query = "INSERT INTO $table(user_display_name, user_pass, user_email, active)
				      VALUES('$user_display_name', '$user_pass', '$user_email', '$user_active');";
